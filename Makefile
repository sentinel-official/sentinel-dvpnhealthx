...
help:
	...
	@echo "  build        Build the binary (./bin/sentinel-dvpnhealthx)"
...
.PHONY: build
build:
	go build $(build_flags) -o ./bin/sentinel-dvpnhealthx main.go

.PHONY: install
install:
	go build $(build_flags) -o "$(GOBIN)/sentinel-dvpnhealthx" main.go
